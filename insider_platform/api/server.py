...
class RegisterRequest(BaseModel):
    ...
    username: str
    password: str
    # Required at signup in the consumer UI.
...
def auth_register(payload: RegisterRequest, response: Response) -> Dict[str, Any]:
    ...
    if not bool(getattr(payload, "accepted_terms", False)):
        # Required for self-serve signups.
    ...
    with connect(cfg.DB_DSN) as conn:
        ...
        # Record legal acceptance (versioned so we can prompt for re-acceptance later).
        now = utcnow_iso()
        terms_version = "2026-03-08"
        conn.execute(
            "UPDATE users SET terms_accepted_at=?, terms_accepted_version=?, updated_at=? WHERE user_id=?",
            (now, terms_version, now, int(u["user_id"])),
        )

        # Refresh public user payload (so the frontend immediately receives the new fields)
        row = conn.execute("SELECT * FROM users WHERE user_id=?", (int(u["user_id"]),)).fetchone()
        if row is not None:
            u = public_user(row)
...
DEFAULT_PROFILE_PREFERENCES: Dict[str, Any] = {
    # Empty list means "no sector filter".
    "preferred_sectors": [],
    # Minimum AI rating (1-10) to consider a signal a "recommendation".
    "min_ai_rating": 7.0,
    # Optional maximum beta filter.
    "max_beta": None,
    # buy | sell | both (buys only by default as requested)
    "trade_side": "buy",
...
def _normalize_profile_prefs(prefs: Any) -> Dict[str, Any]:
    """Parse + merge profile preferences with defaults."""
    base = dict(DEFAULT_PROFILE_PREFERENCES)
...
def _load_profile_row(conn: Any, *, user_id: int) -> Optional[Dict[str, Any]]:
    row = conn.execute("SELECT * FROM user_profiles WHERE user_id=?", (int(user_id),)).fetchone()
    return dict(row) if row is not None else None


def _ensure_profile(conn: Any, *, user: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure a profile row exists for the given user."""
    uid = int(user["user_id"])
    row = _load_profile_row(conn, user_id=uid)
    if row is not None:
        return row

    now = utcnow_iso()
    prefs_json = json.dumps(DEFAULT_PROFILE_PREFERENCES)
    conn.execute(
        """
        INSERT INTO user_profiles (user_id, full_name, contact_email, contact_phone, preferences_json, created_at, updated_at)
        VALUES (?,?,?,?,?,?,?)
        """,
        (
            uid,
    ...
    row2 = _load_profile_row(conn, user_id=uid)
    assert row2 is not None
    return row2


def _profile_payload(row: Dict[str, Any]) -> Dict[str, Any]:
    prefs_raw = row.get("preferences_json") or "{}"
    prefs: Dict[str, Any]
    try:
        prefs = json.loads(prefs_raw) if isinstance(prefs_raw, str) else {}
    except Exception:
        prefs = {}

    prefs = _normalize_profile_prefs(prefs)
...
def get_profile(user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    with connect(cfg.DB_DSN) as conn:
        row = _ensure_profile(conn, user=user)
        return {"profile": _profile_payload(row)}


@app.put("/profile")
def update_profile(payload: UpdateProfileRequest, user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    uid = int(user["user_id"])
    with connect(cfg.DB_DSN) as conn:
        row = _ensure_profile(conn, user=user)

        # Current prefs
        cur_prefs: Dict[str, Any] = {}
        try:
            cur_prefs = json.loads(row.get("preferences_json") or "{}")
        except Exception:
            cur_prefs = {}

        prefs = _normalize_profile_prefs(cur_prefs)

        # Apply updates
        if payload.preferred_sectors is not None:
            prefs["preferred_sectors"] = [str(x).strip() for x in (payload.preferred_sectors or []) if str(x).strip()]

        if payload.min_ai_rating is not None:
            try:
                v = float(payload.min_ai_rating)
            except Exception:
                raise HTTPException(status_code=400, detail="invalid_min_ai_rating")
            if v < 1 or v > 10:
                raise HTTPException(status_code=400, detail="invalid_min_ai_rating")
            prefs["min_ai_rating"] = float(v)

        if payload.max_beta is not None:
            try:
                vb = float(payload.max_beta)
            except Exception:
                raise HTTPException(status_code=400, detail="invalid_max_beta")
            # Allow zero and typical beta ranges, but keep it sane.
            if vb < 0 or vb > 50:
                raise HTTPException(status_code=400, detail="invalid_max_beta")
            prefs["max_beta"] = float(vb)
        elif payload.max_beta is None and "max_beta" in payload.model_fields_set:
            # Explicitly clearing
            prefs["max_beta"] = None

        if payload.trade_side is not None:
            s = (payload.trade_side or "").strip().lower()
            if s not in ("buy", "sell", "both"):
                raise HTTPException(status_code=400, detail="invalid_trade_side")
            prefs["trade_side"] = s

        if payload.email_alerts_enabled is not None:
            prefs["email_alerts_enabled"] = bool(payload.email_alerts_enabled)
        if payload.daily_digest_enabled is not None:
            prefs["daily_digest_enabled"] = bool(payload.daily_digest_enabled)

        full_name = (payload.full_name or "").strip() if payload.full_name is not None else row.get("full_name")
        contact_email = (payload.contact_email or "").strip() if payload.contact_email is not None else row.get("contact_email")
        contact_phone = (payload.contact_phone or "").strip() if payload.contact_phone is not None else row.get("contact_phone")
        ...
        conn.execute(
            ...
            SET full_name=?, contact_email=?, contact_phone=?, preferences_json=?, updated_at=?
            WHERE user_id=?
            """,
            (full_name, contact_email, contact_phone, json.dumps(prefs), now, uid),
        )

        updated = _load_profile_row(conn, user_id=uid)
        assert updated is not None
        return {"profile": _profile_payload(updated)}
...
def admin_delete_user(
    ...
    We prefer deactivation over hard delete to avoid FK issues with support/feedback.
    """

    uid = int(user_id)
    if uid == int(_admin.get("user_id") or -1):
        raise HTTPException(status_code=400, detail="cannot_delete_self")

    now = utcnow_iso()
    with connect(cfg.DB_DSN) as conn:
        row = conn.execute("SELECT user_id, role FROM users WHERE user_id=?", (uid,)).fetchone()
        if row is None:
        ...
        if (row.get("role") == "admin"):
            ...
            if n_admin is not None and int(n_admin["n"] or 0) <= 1:
...
def public_sectors() -> Dict[str, Any]:
    ...
    Used by the Profile page to offer a sector multi-select.
    """
    ...
    with connect(cfg.DB_DSN) as conn:
        ...
        sectors = [str(r["sector"]) for r in rows if r.get("sector")]
...
def list_recommendations(
    ...
    The server applies the user's saved profile preferences:
      - preferred sectors
      - min AI rating (1-10)
      - max beta (optional)
      - trade side (buy/sell/both) — defaults to buys only

    This endpoint is intended to power the "For you" / "Recommendations" page.
    """

    start_date = (date.today() - timedelta(days=int(days))).isoformat()

    with connect(cfg.DB_DSN) as conn:
        # Load profile prefs (create row if missing)
        prow = _ensure_profile(conn, user=user)
        profile = _profile_payload(prow)
        ...
        try:
            min_ai = float(prefs.get("min_ai_rating") if prefs.get("min_ai_rating") is not None else 7.0)
        except Exception:
            min_ai = 7.0
        if min_ai < 1:
            min_ai = 1.0
        ...
        max_beta_f: Optional[float] = None
        if max_beta is not None and max_beta != "":
            try:
                max_beta_f = float(max_beta)
            except Exception:
                max_beta_f = None
        ...
        if isinstance(preferred_sectors, list):
            sectors = [str(s).strip() for s in preferred_sectors if str(s).strip()]

        # Base filters
        where: List[str] = ["e.filing_date >= ?", "(e.has_buy=1 OR e.has_sell=1)"]
        params: List[Any] = [start_date]
        ...
        where.append(f"({best_ai_expr}) IS NOT NULL")
        where.append(f"({best_ai_expr}) >= ?")
        params.append(float(min_ai))
        ...
        # Beta filter (allow NULL beta to pass)
        if max_beta_f is not None:
            where.append("(f.beta IS NULL OR f.beta <= ?)")
            params.append(float(max_beta_f))
        ...
        rows = conn.execute(
            ...
            LEFT JOIN issuer_fundamentals_cache f ON f.ticker = e.ticker
            WHERE {where_sql}
            ORDER BY e.filing_date DESC, COALESCE(({best_ai_expr}), -1) DESC, COALESCE(e.ai_confidence,-1) DESC
            LIMIT ? OFFSET ?
            """,
