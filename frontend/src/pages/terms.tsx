...
Please replace this placeholder with your official Terms of Service.`;
...
export function TermsPage() {
  ...
  const [content, setContent] = React.useState<string>(DEFAULT_TERMS);
  ...
  async function load() {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/backend/public/page/terms`);
      const data = (await res.json()) as PageResponse;
      const md = (data.markdown || "").trim();
      setContent(md || DEFAULT_TERMS);
      setUpdatedAt(data.updated_at_utc || null);
      setDraft(md || DEFAULT_TERMS);
    } catch (e: any) {
  ...
  async function save() {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/api/backend/admin/page/terms`, {
        method: "PUT",
      ...
      setError(e?.message || "Failed to save terms of service");
    } finally {
      setSaving(false);
    }
  }
